...
| Codex CLI rollout JSONL | [official OpenAI Codex rollout/protocol source](https://github.com/openai/codex) | incremental, compressed, exec, and lineage parsing implemented; not product-supported | ephemeral exec has no local file; non-CLI App Server/Desktop sources and product-level UI acceptance remain pending |
| Pi coding agent session JSONL | [official Pi session manager and AI types](https://github.com/badlogic/pi-mono) | incremental parser, flat/nested discovery, fork reconciliation, and provider cost ingestion implemented; not product-supported | product-level health/UI acceptance remains pending |
...
- Official cold rollouts use the same complete JSONL byte stream inside a zstd frame at `.jsonl.zst`. AgentMeter canonicalizes that suffix back to the logical `.jsonl` source key, prefers a plain sibling, and otherwise decodes read-only through the existing parser. Compressed sources always rebuild transactionally because physical byte checkpoints do not correspond to decompressed offsets; corrupt streams fail visibly rather than becoming zero usage.
- Persistent `codex exec` sessions identify `session_meta.source` as `exec` but otherwise use the same rollout contract. `codex exec --ephemeral` and pathless ephemeral App Server threads write no rollout and therefore cannot be recovered from local history. Non-CLI sources such as `vscode` are explicitly skipped by this CLI adapter with a warning so future Codex Desktop support is not mislabeled as CLI usage.
...
Codex remains **not supported** at product level until source-health/i18n/UI acceptance is complete. Persistent CLI and exec rollouts, including official compressed storage, are covered; ephemeral sessions are explicitly unavailable and Desktop/App Server attribution remains a separate future adapter.

## Pi coding-agent session JSONL

- The contract was verified against `badlogic/pi-mono` commit `2509b5c037d366979f2febfce4174b88aeaadc6a` and current session-location history. The current default is `<home>/.pi/agent/sessions/--encoded-project--/*.jsonl`; the retained legacy shape is flat `<home>/.pi/agent/sessions/*.jsonl`. Both use the same JSONL contract, and AgentMeter's recursive read-only scan covers both without claiming that current Pi automatically resumes old flat files.
- The default resolver honors `PI_CODING_AGENT_SESSION_DIR`, then `PI_CODING_AGENT_DIR/sessions`, then `<home>/.pi/agent/sessions`; settings and `--session-dir` are represented by supplying their already resolved flat root. Explicit overrides remain authoritative rather than being combined with an unrelated default root.
...
Pi remains **not supported** at product level until source-health/i18n/UI acceptance criteria pass.
