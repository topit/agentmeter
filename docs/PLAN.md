...
M5-01 (ingestion orchestration) is complete: the app now collects on startup and on demand — discovery, registration, checkpointed ingestion, daily due-reconciliation, and persisted failure diagnostics — with a Sources Rescan command. Next in M5: performance validation against the section 16 targets using synthetic fixture corpora (record sizes and timings in `docs/STATUS.md`), cancellable cold rebuild, and retention (settle open decision 4: keep history after vendor deletion by default). Signing, notarization, and packaging follow once the user's Developer ID certificate is available, then the pre-packaging interactive accessibility pass.
