...
M4-07 is complete: Sessions provides source-scoped immutable summaries with duration, project, client, provider/model, separately labeled provider-reported and API-equivalent cost totals, explicit unpriced events, provenance, and confidence without querying or displaying prompt/response content. Next is the Models/Pricing view over the reviewed rate dataset and reversible estimates, followed by JSON/CSV export. Adapter work stays in the v1.1 backlog; recheck the claude-sonnet-4-7 discount before 2026-09-30.
