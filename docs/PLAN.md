...
M5-05 fixed the first-run experience exposed by the first real DMG test: collection itself was proven correct (593 Codex rollouts → 74,682 events), and the app now shows live scan progress (processed/discovered counts on Sources) plus a localized collecting card on Overview instead of misleading empty states. Next: re-cut the DMG, verify a fresh-database first run end to end, then finish M5 with codesign/notarytool (awaiting the Developer ID certificate), the pre-packaging accessibility pass (awaiting the local Metal Toolchain repair), and the privacy review plus README refresh.
