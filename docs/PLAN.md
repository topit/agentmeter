...
M4-04 (pricing snapshots and reversible estimates) is complete: integer nano-USD estimation, reviewed matching precedence, snapshot idempotence, and wholesale-reversible repricing across the pricing/storage/application layers, with the bundled dataset deliberately empty until rates are reviewed. Next, in order: seed the reviewed rate dataset from official provider pricing pages (every rate keeps its documentation evidence), then M4-05 the Activity view (daily/weekly/monthly aggregation, tokens/cost toggle, stack/group dimensions), followed by the Sessions, Models/Pricing views and JSON/CSV export. Adapter work stays in the v1.1 backlog.
