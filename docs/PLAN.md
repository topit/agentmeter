...
M5-02 (performance validation) is complete: a 100k-event synthetic corpus on the user's Apple Silicon Mac meets the section 16 targets — warm dashboard 110 ms (<200 ms), unchanged warm reconciliation 45.7 ms (<1 s), append refresh proportional — after appends switched to transactional incremental daily projection. Next in M5: cancellable cold rebuild (a cancellation token through the ingestion service so a long scan can be abandoned without corrupting checkpoints), then retention (settle open decision 4: keep history after vendor deletion by default), then signing/packaging with the user's Developer ID certificate and the pre-packaging accessibility pass.
